...
{   
    ...
    for (int i = coords.x - 1; i < coords.x + offset_x + 2; i++) {
        if (i >= 0 && i < size_x_) {
            for (int j = coords.y - 1; j < coords.y + offset_y + 2; j++) {
                if (j >= 0 && j < size_y_) {
...
FieldCellStatus Field::getCellStatus(Coords coords) const
{
    if (coords.x < 0 || coords.x >= size_x_ || coords.y < 0 || coords.y >= size_y_) {
        throw std::out_of_range("Coordinates out of range");
    }

    if (opened_cells_.count(coords) == 0) {
        return FieldCellStatus::unknown;
    } else {
        return opened_cells_.at(coords).getStatus();
    }
}

ShipSegmentStatus Field::getShipSegmentStatus(Coords coords) const
{
    if (ships_.count(coords) == 0) {
        throw std::out_of_range("No ship at such coordinates");
    }

    return ships_.at(coords).getShipSegmentStatus();
}
