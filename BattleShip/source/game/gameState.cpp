...
std::ifstream& operator>>(std::ifstream& in, GameState& state) 
{
    ...
    if (!check_res) {
        std::cout << "Save file corrupted" << std::endl;
        return in;
    }
    ...
    for (int i = 0; i < state.player_count_ + state.bot_count_; i++) {
        ...
        if (player_or_bot == 0) {
            ...
            player->ability_manager = ability_manager;
            participant = player;
        } else {
            participant = new Bot();
        }
...
std::ofstream& operator<<(std::ofstream& out, GameState& state) 
{
    ...
    for (int i = 0; i < state.participants_.size(); i++) {
        auto player = dynamic_cast<Player*>(state.participants_[i]);
        if (player) {
            data += "0\n";
            data += player->ability_manager->getAllAbilities();
        } else {
            data += "1\n";
        }

        data += state.participants_[i]->field->getField() + state.participants_[i]->field->getShips();
    }
